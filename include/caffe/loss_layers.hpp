...
class WeightedEuclideanLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  /*void Apply_weights_gpu(Blob<Dtype>& blob, Blob<Dtype>& bottomWeights);*/
...
// Assumes an output range of -1,1 for tanh units. DONT USE WITH SIGMOID
template <typename Dtype>
class NonBinaryPenaltyLossLayer : public LossLayer<Dtype> {
 public:
  explicit NonBinaryPenaltyLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), non_binary_penalty_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "NonBinaryPenaltyLoss"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
  // Just needs an output vector
  virtual inline int ExactNumBottomBlobs() const { return 1; }
 protected:
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
